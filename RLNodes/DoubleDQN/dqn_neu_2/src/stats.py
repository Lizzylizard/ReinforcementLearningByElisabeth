...
# set info
def set_info(state, reward, action, net_number):
  global states, rewards, actions, net_num
  states.append(state)
...
def save_records():
  ...
  try:
    # open correct file
    f = open(path + "actions_states_rewards" + str(net_num)+ ".txt",
      "w")
  ...
  with open (path + str(net_num) + "reward_arr", 'wb') as fp:
    pickle.dump(rewards, fp)

  with open (path + str(net_num) + "action_arr", 'wb') as fp2:
    pickle.dump(actions, fp2)

  with open (path + str(net_num) + "states_arr", 'wb') as fp3:
...
def create_diagrams():
  ...
  plt.xlabel('Episode')
  path_2 = path + str(net_num) + 'reward_per_episode' + str(cnt) + \
           '.png'
  if ((cnt % 10 == 0) or (cnt == 1)):
    plt.savefig(path_2)
  plt.close()
